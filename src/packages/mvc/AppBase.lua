...
function AppBase:ctor(configs)
    ...
    if DEBUG > 1 then
        print_table(self.configs_, "AppBase configs")
    end
...
function AppBase:enterScene(sceneName, transition, time, more, params)
    local view = self:createView(sceneName)
    view:showWithScene(transition, time, more, params)
    return view
end

function AppBase:createView(name, options, data)
    for _, root in ipairs(self.configs_.viewsRoot) do
        ...
        if status and (t == "table" or t == "userdata") then
            return view:create(self, name, options, data)
        end
    end
